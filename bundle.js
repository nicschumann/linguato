...
var require_regl = __commonJS((exports2, module2) => {
  (function(global, factory) {
    ...
    function parseArgs(args_) {
      var args = args_ || {};
      var element, container2, canvas, gl;
      ...
      if (!gl) {
        if (!canvas) {
          ...
          var result = createCanvas(container2 || document.body, onDone, pixelRatio);
          if (!result) {
            return null;
          }
...
var u_mouse_x_key = "linguato__u_mouse_x";
var mouse_active = false;
var u_mouse_x = Number(localStorage.getItem(u_mouse_x_key)) || 0;
var smile_target = document.getElementById("metadata-parameter--mouth-smile--value");
var openness_target = document.getElementById("metadata-parameter--mouth-openness--value");
var width_target = document.getElementById("metadata-parameter--mouth-width--value");
var u_mouse_width = 0;
var u_mouse_open = 0;
var u_mouse_smile = 0;
...
window.addEventListener("mousewheel", (e) => {
  e.preventDefault();
  u_mouse_smile = Math.min(Math.max(u_mouse_smile + e.deltaY / window.innerHeight, -1), 1);
  smile_target.innerText = u_mouse_smile.toFixed(2);
});
...
window.addEventListener("mousemove", (e) => {
  if (mouse_active) {
    ...
    u_mouse_width = Math.min(Math.max(e.clientX / window.innerWidth, 0), 1);
    u_mouse_open = Math.min(Math.max(e.clientY / window.innerHeight, 0), 1);
    width_target.innerText = u_mouse_width.toFixed(2);
    openness_target.innerText = u_mouse_open.toFixed(2);
  }
});
